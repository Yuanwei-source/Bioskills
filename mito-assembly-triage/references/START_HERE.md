...
| 需要一个参考 GenBank | 按参考等级选一条**已注释**公共记录（§5.1）；用 `reference_registry.py` 登记 accession/版本/日期/类群/hash/用途（政策见 `reference-policy.md`） | 顺序、边界、基因身份的对照 | 参考不是真值；**上传样本**才是需要单独授权的动作 |
