...
| COX1 查询 | `scripts/cox1_id.py <genome.fasta> --allow-public-upload --coords start,end [--max-results 5]` | **必须**显式给 `--coords`（不自动识别 COX1）；查询 400–5000 bp；**会向公共 NCBI 上传该片段** | 候选取样片的 identity、**query coverage**（多 HSP query 区间并集）、多个候选与 `provisional_candidate`/`ambiguous`/`insufficient` 判读；使用 `FORMAT_TYPE=XML2` 结构化解析；保留原始 HSP 与非冗余覆盖 | identity/coverage **不等于**物种鉴定；**HSP 区间重叠时记 `AMBIGUOUS_ALIGNMENT` 且不汇总单一 identity**；覆盖率 <80% 或 top 与次优相差 <1% 时不给结论；阈值只是候选过滤启发式 | 上传到 NCBI；退出码 `0` 得判读 / `1` insufficient或no_match / `2` 拒绝执行 / `3` 网络或结果格式故障 |
