...
| 注释质检 | `scripts/annot_check.py <ann.gb> [--ref <ref.gb>] [--table 5] [--require-circular] [--tolerate-overlap "G1,G2"] [--tolerate-start "GENE:CODON"] [--overlap-severity error\|warn] [--allow-atypical "<理由>"]` | 带注释的 GenBank | 基因数量/身份/重复、tRNA 类型是否可判定、CDS 翻译与起始终止、`/codon_start` 5' partial、tRNA/rRNA 长度、**逐对**重叠（分段坐标）、链分布与方向、（可选）顺序与长度对比 | 注释内部一致 ≠ 序列被 reads 支持（脚本结尾固定打印该免责声明） | 只读；`0` 无发现 / `1` 有错误（含参数错误）/ `2` 仅待核查 |
...
| COX1 查询 | `scripts/cox1_id.py <genome.fasta> --allow-public-upload --coords start,end [--max-results 5]` | **必须**显式给 `--coords`（不自动识别 COX1）；查询 400–5000 bp；**会向公共 NCBI 上传该片段** | 候选取样片的 identity、**query coverage**、多个候选与 `provisional_candidate`/`ambiguous`/`insufficient` 判读 | identity/coverage **不等于**物种鉴定；覆盖率 <80% 或 top 与次优相差 <1% 时不给结论 | 上传到 NCBI；无合格命中 → `1` |
